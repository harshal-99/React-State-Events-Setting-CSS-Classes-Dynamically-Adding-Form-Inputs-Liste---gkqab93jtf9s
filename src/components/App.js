...
const Towns = ({ towns, setCurrentTown }) => {
	const handleTownChange = (event) => {
		setCurrentTown(parseInt(event.target.value))
	}
	return (
		<select id="landmark" onChange={handleTownChange}>
			{towns
				.map((town, index) =>
					<option value={index} key={town.name}>{town.name}</option>
				)}
		</select>
	)
}
...
const Cities = ({ cities, setCurrentCity, setCurrentTown }) => {
	const handleCityChange = (event) => {
		setCurrentCity(parseInt(event.target.value))
		setCurrentTown(0)
	}
	return (
...
const States = ({ states, setCurrentState, setCurrentCity, setCurrentTown }) => {
	...
	return (
		<select id="state" onChange={handleStateChange}>
			{states
				.map((state, index) =>
					<option key={state.name} value={index}>{state.name}</option>)}
		</select>
	)
}
		...
		<div id="main">
			<div className="control">
				<div className="control__items">
					...
					<States states={states} setCurrentState={setCurrentState} setCurrentCity={setCurrentCity} setCurrentTown={setCurrentTown} />
				</div>
				<div className="control__items">
					<label htmlFor="city">Cities:</label>
					<Cities cities={states[currentState].city} setCurrentCity={setCurrentCity} setCurrentTown={setCurrentTown} />
				</div>
				<div className="control__items">
					<label htmlFor="landmark">Towns:</label>
					<Towns towns={states[currentState].city[currentCity].landmarks} setCurrentTown={setCurrentTown} />
				</div>
			</div>
			<div className="info">
				<Card location="state" name={states[currentState].name}
					description={states[currentState].description} />
				<Card location="city"
					name={states[currentState].city[currentCity].name}
					description={states[currentState].city[currentCity].description} />
				<Card location="landmark"
					name={states[currentState].city[currentCity].landmarks[currentTown].name}
					description={states[currentState].city[currentCity].landmarks[currentTown].description} />
			</div>
		</div>
	);
}
